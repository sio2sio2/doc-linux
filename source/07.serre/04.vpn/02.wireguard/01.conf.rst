...
.. important:: A partir de *Debian Trixie* el servicio :program:`systemd-sysctl`
   de :ref:`SystemD <systemd>` ha dejado de leer el archivo :file:`/etc/sysctl.conf`,
   por lo que éste no existe (véanse `las notas de lanzamiento de la versión
   <https://www.debian.org/releases/trixie/release-notes/issues.html#etc-sysctl-conf-is-no-longer-honored>`_)
   y obrar del modo indicado arriba creando el archivo sólo nos servirá para
   configurar el parámetro del núcleo en esa ocasión, pero no en los reinicios.
   A partir de esta versión debe crearse un archivo dentro de
   :file:`/etc/sysctl.d`::

      # echo "net.ipv4.ip_forward = 1" >> /etc/sysctl.d/99-override.conf
      # sysctl -p /etc/sysctl.d/99-override.conf  # También: system-ctl --system
