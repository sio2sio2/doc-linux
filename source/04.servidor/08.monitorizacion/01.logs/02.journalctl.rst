...
- Los registros se pasan también al demonio :command:`rsyslogd` para que este
  ...
  programas usan el sistema tradicional de *logs* para obtener datos\ [#]_.
...
.. hint:: Si no ha revisado el :ref:`apartado dedicado al sistema tradicional
   de registros <rsyslog>`, le recomendamos que consulte al menos :ref:`cómo se
   clasifican los mensajes según su categoría y su nivel de gravedad
   <message-class>`.
...
* Seleccionar mensajes filtrando con una :ref:`expresión regular PCRE <regex>`::
  ...
  No hace falta, pues, desviar la salida a :ref:`grep <grep>` con una tubería.
...
.. [#] Siempre que el paquete :deb:`rsyslog` esté instalado en el sistema.
   A partir de *Bookworm*, la instalación estándar no lo incluye, pero si se
   instala manualmente comenzará a registrar sin más configuración, porque
   :program:`SystemD` habilita el reenvío de mensajes a
   :file:`/run/systemd/journal/syslog` por defecto, ya que la configuración
   predeterminada es :code:`ForwardToSyslog=yes` (véase :manpage:`journald.conf`).
   Es en ese *socket* (y no en el tradicional :file:`/dev/log` que ahora se
   reserva *journald*)  donde los sistemas modernos hacen escuchar a
   :program:`rsyslog`::

        $ journalctl --grep imuxsock
