...
#. Paquetería universal:
   ...
   *software* ya instalado a través de la paquetería propia de la distribución;
   ...
   + Snap_, promovida por *Canonical*, la empresa detrás de *Ubuntu*. No dispone
     de esa base común, de manera que tiende a incluir todas sus dependencias
     dentro del paquete. El principal problema de esta alternativa es que
     *Canonical* controla el repositorio del que se descargan los paquetes de
     *Snap* y resulta imposible que terceros creen repositorios alternativos, lo
     que convierte a esta paquetería en un monopolio de *Canonical* que puede
     hacer y deshacer a su antojo (como *Google* puede hacerlo en la *Play
     Store* o Apple en la *Apple Store*).
   + AppImage_, que desarrolla el mismo concepto que los `bundle de MacOS
     <https://en.wikipedia.org/wiki/Bundle_(macOS)>`_: el archivo contiene todas
     las dependencias necesarias para que se ejecute la aplicación y no se
     instala: al ejecutarlo se monta su contenido y arranca el programa.
     Eliminar la aplicación consiste básicamente en eliminar el archivo.
...
#. Instalación manual:
   ...
   + En ocasiones, se nos ofrecen en forma de *paquete* del sistema propio
     ...
     entenderemos) y se carecerá de actualización a través del gestor de
     paquetes.
...
*paquete* que conviene tener claro antes de proseguir:

:dfn:`Paquete`
   Es una colección de archivos binarios, de documentación o de configuración
...
:dfn:`Sistema de paquetes` (o :dfn:`sistema de paquetería`)
   ...
   accesibles a través de *repositorios*, que se estructuran de modo que se
   facilite su instalación, mantenimiento y actualización.

:dfn:`Repositorio` o :dfn:`réplica`
   Es un soporte (cdrom, servidor web, servidor |FTP|) donde se almacenan
...
Por último, es importante tener claro que los paquetes (al menos los de
distribución) no son independientes unos de otros, sino que existe una compleja
relación de dependencias entre ellos, ya que se empaqueta las librerías que
usan los programnas  y los programas en paquetes separados, a fin de que las
dependencias (o sea, las librerías) no se instalen repetidamente. De este modo,
los dos paquetes de sendas aplicaciones finales, que dependan de una misma
librería, no requerirán incluir dicha librería dentro de sí, sino únicamente
una dependencia al paquete que instala la librería. Esto elimina, pues,
duplicidades, pero introduce relaciones entre los distintos paquetes y provoca
que el sistema de paquetes evoluciones como un conjunto. Así, la actualización
a una nueva versión de esa librería, provocará que los paquetes de esas dos
aplicaciones finales relacionadas también se actualicen.
...
:dfn:`Multigestores` (terminología propia)
   ...
   + `Gnome Software`_, agregador para el escritorio *Gnome*, que permite
     compatibilizar los paquetes de Snap_ y Flatpak_ con los de la distribución
     en la que se instale. Otros agregadores semejantes como :program:`Ubuntu
...
Stretch_; y la inestable es la única que no cambia su nombre y se llama siempre
...
**apt**
   ...
   operación que queramos realizar, debemos usar uno u otro comando
   (:manpage:`apt-get`, :manpage:`apt-cache`). Para paliar esto, se creó la orden
   :command:`apt` a secas que reúne las operaciones más habituales.
   Modernamente, es la herramienta recomendada.
...
**aptitude**
   ...
   A partir de Stretch_ no viene instalado de serie.
...
internet, lo que exige tener definidos los repositorios en el archivo
:file:`/etc/apt/sources.list`. Un contenido típico es el siguiente:

.. include:: docs/sources.list
   :literal:

Cada línea del archivo representa una fuente de obtención de paquetes y su
...
Además de este archivo, puedes crearse otros ``.list`` dentro de
...
.. caution:: La acción anterior no actualiza el sistema, sólo la lista de
...
#. Hacerlo de modo que no se eliminen los archivos de configuración::
...
o a qué paquete pertenece un determinado archivo. Por ejemplo, si quisiéramos
...
.. note:: Obsérvese que se ha preferido incluir la ruta completa
   (:file:`/usr/bin/python3`, que es la salida de :code:`which python3`) frente
   al nombre solo (*python3*). Esto es debido a que la búsqueda funciona de tal
   forma que devuelve cualquier paquete que contenga un archivo en cuya ruta
   aparezca la palabra *python3* y hay muchos archivos en el sistema que cumplen
   esto. Por tanto, si hubiéramos hecho :code:`dkpg -S python3` se habrían
   devuelto infinidad de resultados. Hágase la prueba.

.. caution:: A partir de Buster_, no existe el directorio :file:`/bin`, sino que
   éste es un enlace simbólico a :file:`/usr/bin`. Esto es un problema, cuando
   buscamos el paquete al que pertenece un ejecutable instalado en :file:`/bin`,
   puesto que en la variable *PATH* :file:`/usr/bin` aparece antes y, en
   consecuencia, será esta ruta la que nos devuelva :ref:`which <which>`.
   Como en el contenido de los paquetes (que es donde busca :command:`dpkg`)
   aparece la ruta :file:`/bin/`, se nos dirá equivocadamente que no hay ningún
   paquete que contenga el ejecutable::
   ...
   Por ese motivo, se ha añadido la opción :kbd:`-a` que devuelve todos los
   ejecutables que se encuentran con tal nombre y no solamente el primero::
   ...
   Ahora, al menos, sí se localizará el paquete, aunque siga mostrando el error.
...
En este último caso, se devolverán todos los paquetes que requieran instalar
aquel por el que se pregunta, estén o no instalados en el sistema. Lo habitual es que,
cuando hagamos la pregunta, nos interese conocer sólo los paquetes realmente
instalados, porque nuestra intención es conocer qué paquete es el responsable de
que tengamos algo instalado. Por ejemplo::

   $ apt list policykit-1
   Listando... Hecho
   policykit-1/testing,now 122-3 amd64 [instalado, automático]

En mi sistema de escritorio se encuentra instalado el paquete :deb:`policykit-1`
que no instalé explícitamente, puesto que aparece como automático. Como nunca
pedí instalar sugerencias al utilizar :program:`apt`, no necesito la
información sobre sugerencias (ni reemplazos, etc. que tampoco me aportan
mucho), sino estrictamente las dependencias y recomendaciones. En este caso,
averiguar qué paquete provocó la instalación de éste se logra así::
...
Ya lo tenemos: es :deb:`rtkit`, pero::

   $ apt list rtkit
   Listando... Hecho
   rtkit/testing,now 0.13-5 amd64 [instalado, automático]

éste paquete también se instaló automáticamente. Podríamos repetir la operación
indagando sobre él, pero en vez de eso podemos añadir la opción :kbd:`--recurse`
...
:deb:`rtkit` se instaló porque fue una recomendación de :deb:`pulseaudio` y éste
quizás porque es a su vez una recomendación de :deb:`pavucontrol`::
...
En realidad, no fue así, sino que ambos se instalaron a mano (aunque podría
haber ocurrido que el único instalado explicitamente fuera *pavucontrol*\ [#]_).
Por tanto, mientras *pavucontrol* (y *pulseaudio*) se encuentre en el sistema,
*policykit-1* no podrá desinstalarse\ [#]_.
...
"""""""""""""""""""""""""""
Es posible automatizar la actualización de paquetes de modo que no se requiera
intervención humana. Esto tiene una gran ventaja y un gran inconveniente:

* La **ventaja** es que tendremos siempre actualizado el sistema sin tener que
  preocuparnos.
* La **desventaja** es que no controlamos cuándo se actualizan los paquetes y
  la actualización a una nueva versión puede introducir incompatibilidades con
  la configuración que hayamos hecho. Por eso motivo, no es **nada
  recomendable** activar esta posibilidad si corremos una rama distinta de la
  estable.

La automatización se logra gracias al paquete :deb:`unattended-upgrades`, para
el que tenemos unas `instrucciones en la wiki de Debian
<https://wiki.debian.org/UnattendedUpgrades>`.

.. todo:: Trasladar las instrucciones de  *unattended-upgrades*.
...
El archivo, tal y como está, guarda bastante equilibrio: instalaremos paquetes
...
Esto se hace modificando (o creando el archivo :file:`/etc/apt/preferences`\
...
Línea **Pin**
   ...
   * Las propiedades de la distribución a la que pertenece el paquete. Son
     diversas y se pueden consultar el los archivos ``*Release``, citados
     en una de las notas al pie. Se enumeran aquí algunas propiedades,
     expresando el nombre del campo con que se enuncian en tal archivo:
...
.. note:: El directorio :file:`/usr/src` quedará lleno de archivos relacionados
...
archivos que tenemos instalados en el disco duro, bien porque no encontramos
cómo hacernos con el ``.deb`` original, bien porque hemos cambiado sus archivos
...
Los archivos ``.deb`` son empaquetados comprimidos de archivos que, además de
...
Consigámoslo, descargando el archivo con :program:`apt-get`, pero sin llegar a
instalarlo::

   # apt install -d pv

La orden habrá almacenado el archivo descargado en
:file:`/var/cache/apt/archives`. Podemos analizar el contenido de este archivo
...
Contiene tres archivos, el primero guarda simplemente la versión del formato
``.deb`` y los otros dos son archivos empaquetados y comprimidos, uno
(:file:`data.tar.xz`) guarda los archivos en sí que constituyen el paquete  y el
otro (:file:`control.tar.gz`) los archivos de metainformación, es decir, los
archivos que sirven para identificar cuál es el paquete, qué archivos debe
contener o qué acciones hay que realizar cuando se instala o se elimina el
paquete del sistema. No entraremos a describir cuáles son estos archivos
...
:command:`dpkg` tiene las herramientas necesarias para desempaquetar el archivo::
...
operación descomprime los archivos en sí dentro de tal directorio, de manera que
...
:file:`DEBIAN` los archivos de metainformación. En este punto podríamos realizar
modificaciones de tanto de los archivos de datos como de los de control (¡ojo!
que el archivo de control :file:`md5sums` almacena las comprobaciones de la
integridad de los archivos).

Si en algún momnento quisiéramos reconstruir el archivo, podríamos hacer lo
siguiente::
...
.. note::
   En realidad, conociendo cuáles son los archivos de control y cómo se
   escriben, es posible crear nuestros propios archivos ``.deb``. Basta con
   reproducir la estructura que hemos visto al descomprimir el archivo y
...
.. [#] Esta necesidad será mayor en las :ref:`distribuciones con liberación de
   versiones <ciclo-distro>` que en las de *liberación continua*.

.. [#] Existen otros formatos de archivos y, en general, cada distribución
   *madre*  tiene el suyo propio: *Red Hat* (y *SuSE*) usa archivos :kbd:`.rpm`,
   *Archinux*, archivos :kbd:`.pkg.tar.xz`; *Gentoo*, paquetes :kbd:`.ebuild`; y
   *Slackware*, :kbd:`.tgz`.
...
.. [#] En la revisión del epígrafe, la versión estable es la **10**, Buster_; y
   las posteriores se llamarán Bullseye_ (**11**), *Bookworm* (**12**) y
   *Trixie* (**13**).
...
.. [#] La diferencia entre tener los nombres clave o los nombres de la rama está
   ...
   (Stretch_), tendremos instalada en nuestra máquina la distribuciín en pruebas,
...
.. [#] *pulseaudio* es un servidor de sonido que puede utilizarse en la salida
   de numerosos programas (aunque no es estrictamente necesario).
   *pavucontrol*, por su parte, es el mezclador para este servidor. Obviamente,
   es absurdo instalar el mezclador, si no se tiene instalado el servidor,
   razón por la cual ess una recomendación. Sin embargo, el servidor funciona
   perfectamente sin el mezclador. Esa es la razón por la que ambos paquetes
   están instalados manualmente: muy probablemente se instaló primero
   manualmente *pulseaudio* que funcionó perfectamente y, sólo tiempo después,
   quizás para tener mayor control sobre él, se quiso usar el mezclador.

.. [#] Es importante tener presente que un paquete instalado automáticamente se
   instaló originariamente porque era dependencia  (directa o indirecta) de otro
   cuya instalación se pidió expresamente. Sin embargo, tiempo después puede
   requerirse la instalación de un paquete distinto del que también es
   dependencia. En este caso, habrá dos paquetes que son los responsables
   últimos del paquete instalado automáticamente y ya será imposible saber cuál
   fue originariamente el que provocó su instalación. En cualquier caso, cuál
   fuera el culpable original tiene poca importancia: lo relevante es que la
   existencia de ambos provoca que esa dependencia esté instalada y mientras
   cualquiera de los dos permanenzca instalado, no será posible borrar esa
   dependencia.
...
.. [#] Para saber cuáles son las opciones que tiene fijadas una rama se puede ir
   ...
   archivos descargados por la orden :code:`apt-get update`. Cada línea del
   *sources.list* genera varios archivos uno de los cuales tiene por nombre
...
.. [#] Como es habitual, también existe un directorio llamado
   :file:`/etc/apt/preferences.d` en el que podemos crear diferentes archivos
...
.. _AppImage: https://appimage.github.io/
