...
:command:`umask`
   ...
   :command:`umask` también acepta argumento con letras como :ref:`chmod
   <chmod>`. En este caso, sin embargo, lo que se expresa son los permisos que
   realmente queremos que tengan los archivo. Por ejemplo, partiendo de::

      $ umask
      0022

   Ya sabemos que obtenemos permisos ``rw-r--r--`` para archivos. Si quisiéramos
   añadir a la máscara el permiso de escritura para el grupo propietario
   podríamos hacer:
   ...
   Ahora, cualquier nuevo archivo se creará con permisos ``rw-rw-r--``.
