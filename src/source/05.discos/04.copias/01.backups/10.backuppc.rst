...
.. warning:: :program:`BackupPC` también envía mensajes de correo a ciertos
   uusarios, por lo que requiere e instala automáticamente un agente de
...
.. seealso:: Para profundizar en la instalación del servidor web debería echar
   un vistazo al :ref:`epígrafe dedicado al servico web <web>`.
...
Almacen de copias
-----------------
Las copias de seguridad se almacen dentro del directorio
...
La primera es el nombre de máquina, que tendrá que coincider con un nombre
*Netbios* o un nombre de red resoluble por el servidor; la segunda se recomienda
dejarla a 0; la tercera es el usuario al que se advertirá via correo electrónico
...
- Para que el usuario de la primera columna reciba correo, :program:`BackupPC`
  ...
  lugar :ref:`postfix <smtp>`; y, si sólo queremosa tener la posibilidad de
...
Linux
-----
Sin perjucio de que se pueda usar otro método, para el acceso a los clientes se
...
Restauración
============
Para la restauración de un respaldo, lo más cónodo es utilizar la interfaz web
...
+ Al menos cuando se llevan a cabo una restauración con :command:`tar` siguiendo
  ...
  estropea el sistema final y toca reparlos reajustando los permisos
