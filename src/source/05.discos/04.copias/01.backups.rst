...
* La explicación de algunos mecanismos para copias de máquinas indivuales.
* La explicación de algunos mecanimos para copias centralizadas de redes de
  ordenadores.
...
#. Qué **destino** se dará a la copia: cuánto mayor sea la distancia entre
...
#. En un sistema en red con varios dispositivos, la **centralización** del
   proceso o, por el contrario, el que se lleven a cabo 
...
* **Notificar** su resultado (de buen o mal suceso) al administración, por lo
...
#) Según el **origen** de los datos y el **destino** de la copia (**destino**):
   ...
   + En `cintas magnéticas
     ...
     <https://es.wikipedia.org/wiki/Linear_Tape-Ope>`_. Son de acceso secuencial
     y lentas pero, en contraposición, muy baratas por lo que se siguen usando en
     los ambientes empresariales cuando el volumen de datos es alto. De hecho,
     el nombre de la herramienta (y formato de *backup*) :ref:`tar <tar>` es un
     acrónimo de *Tar ARchive*. Las cintas suelen cargarse en dispositivos
     independientes del equipo, incluso conectados mediante red y no
     directamente, e incluso pueden cargarse y reutilizarse automáticamente, por
     lo que pueden cumplir los requisitos exigidos a cualquier sistema
     profesional de copias.
...
#) Según la **naturaleza de los datos** (**contenido**):
   ...
   + :dfn:`Copia de bases de datos`, que son aquellas copias que respaldan datos
     almacenados en bases de datos. En principio, estos son tambian datos
     generados, pero al estar estructurados en bases de datos suelen usarsa
   ...
   + :dfn:`Copia del sistema y las aplicaciones`, que son aquellas que respaldan
     ...
     *copias imagen* para las que se ha usado un software específico de clonado.
     En el mundo del ŝoftware libre* son muy habituales Clonezilla_ y Fog_.
     ...
     de una actualización o modificación de las configuración del sistema.
...
#) Según los datos a copiar **desde la última copia** (**contenido**):
   ...
   + :dfn:`Copia completa`, que es aquella en la que se incluyen todos los datos
     que se desean repaldar, con independencia de las copias que se hayan podido
   ...
   * :dfn:`Copia diferencial`, que es aquella en la que se incluyen todos los
     ...
     respecto a la anterior de que tarda menos tiempo en llevarse cabo yh ocupa
     ...
     tiempos entonces las copias diferenciales incluirá idénticos archivos. Por
   ...
   * :dfn:`Copia incremental`, que es aquella en la que se incluyen todos los
   ...
   Lo habitual es que la politica de copias implemente un sistema mixto en que
   se haga una *copia completa* cada cierto tiempo y, entre ellas, copias
...
#) Según la **centralización** del proceso:
   ...
   Dado que divideremos las herramientas de copia según este criterio,
...
- La política de **rotación** de las copias, esto es, a partir de qué copia
  antigua copia se borran las copias anteriores.
...
- Muy propablemente, :ref:`un script <scripts>` que lance la herramienta
...
Copia centralizada
==================
Los sistema de :dfn:`copia centralizada` permiten definir en un servidor una
