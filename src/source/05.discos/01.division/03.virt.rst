...
de otro volumen lógico. Por ejemplo, vamos a dar a uno de los volúmenes lógicos
y a escribir dentro de él::

   # mkfs.ext4 -L PRIMERA /dev/Vtest/primera
   # mount /dev/VGtest/primera /mnt
   # echo "Hola" > /mnt/saluto.txt

Hecho lo cual, podemos crear una instatánea del estado actual de "primera"::

   # lvcreate -s -n primera_snapshot -L 50M /dev/VGtest/primera
...
definir la instantánea.

A partir de ahora puede alterarse el contenido de :file:`/dev/VGtest/primera`::

   # echo "Adios" >> /mnt/saludo.txt

Podemos controlar cuál el tamaño ocupado por las copias en la instatánea con
:ref:`lvs <lvs>`::

   # lvs /dev/VGtest/primera_snapshot

porque hay que tener presente que la instatánea deja de tener utilidad si se
queda sin espacio. Para aumentarlo, puede usarse :ref:`lvextend <lvextend>` como
se hace con los volúmenes lógicos normales. Si en algún momento se dean revertir
todos los cambios y volver al estado de la instantánea::

   # lvconvert --merge /dev/VGtest/primera_snapshot

volveremos a él en cuanto volvamos a montar el volumen lógico::

   # mount -o remount /mnt

Si decidimos que no tiene sentido mantener la instánea, simplemente, borramos el
volumen::

   # lvremote /dev/VGtest/primera_snapshot
