...
.. rubric:: La firma en documentos |PDF|

Tal y como la hemos presentado aquí, la firma es un archivo externo al propio
archivo firmado. Los archivos |PDF|, sin embargo, permiten incluir un campo con
la propia firma del contenido, de ahí que la firma no se proporcione en archivo
independiente, sino incrutada en el propio archivo.
...
Lo habitual es que se usen claves |PGP| un estándar desarrollado para el cifrado
y firma de mensajes de correos electrónicos, cuya generación, importación y
exportación de claves se :ref:`practicamos al tratar el cifrado asimétrico
<gpg-pgp>`. Obviamente, el proceso de firma de un mensaje no exige la
elaboración *artesanal* de la firma, sino que los clientes de correo como
:ref:`mutt <mutt>` o Thunderbird_ suelen incorporar esa funcionalidad para que
...
propietario de tal clave publica, hay aún una debilidad: ¿cómo puede el
...
Un :dfn:`certificado digital` es un documento compuesto por unos datos de
identificación y una clave pública, y firmado digitalmente por una *autoridad de
certificación*, que acredita que la clave pertenece a quien indican tales datos
identificativos. Si esta autodiad de certificación (|CA|) es de absoluta
confianza entonces no habrá duda de quién es el propietario de la clave pública
y desaparecerá el riesgo de suplantación. LOs certificados suelen basarse en el
...
avalado por la firma de otra |CA| de rango superior. Sin embargo, en la cúspide
se encuentran las |CA| raíz, cuyos certificados son autofirmados, lo que
...
habrán avalado de algún modo su identidad. Así, *Debian* incluye el paquete
...
web <nginx-https>` emitido por `Let's Encrypt`_, cuyo certificado a
su vez esta firmado por `Digital Signature Trust`_, que sí es una |CA| raíz::

   $ openssl x509 -in letsencrypt.cer -subject -issuer -noout
...
Los certificados, no obstante, no son eternos: la |CA| los acredita por un plazo
determinado, por lo que son sólo válidos entre una fecha inicial (normalmente la
fecha en que se generan) y una fecha final que varía según el tiempo de validez
por el que se certifiquen. Ambas fechas se incluyen dentro de los datos
identificativos.
...
autoridad teleacredite al servidor.

El procedimiento para obtener certificados de servidor mediante el protocolo
|ACME| se encuentra descrito en :ref:`este epígrafe sobre nginx <nginx-https>`.
...
|PEM|
   ...
   Este estándar genera archivos imprimibles (está codificado en Base64_) y
   suele presentarse bajo estas extensiones:
   ...
   * ``.pem`` tanto cuando contienen sólo el certificado público (y,
     ...
     contienen además el privado. 
   * ``.key`` cuando el fichero contiene sólo la parte privada.
   * ``.cer`` y ``.crt`` cuando contiene sólo la parte pública. Frecuentemente,
     sin embargo, el formato de este fichero no es |PEM| sino |DER|.
   ...
   #. Transformar el certificado público |PEM| a formato |DER|::

         # openssl x509 -inform pem -in CERTIFICADO.pem -outform der -out CERTIFICADO.crt

   #. Trasformar la clave privada |PEM| al formato |DER|::

         # openssl rsa -inform pem -in CERTIFICADO.key -outform der -out CERTIFICADO.key.der
   ...
   #. Generar un archivo ``.p12`` a partir de los archivos ``.pem`` y ``,key``::
         ...
         # openssl pkcs12 -export -out CERTIFICADO.p12 -in CERTIFICADO.pem -inkey CERTIFICADO.pem
      ...
      :kbd:`-inkey`.  La archivo con la parte pública debe contener al menos la
      ...
      .. note:: Como se necesita introducir una clave para descifrar el
         ...
         :file:`CERTIFICADO.p12`\ [#]_ se pedirán interactivamente las dos claves. Si
         se quieren pasarlas por la línea de órdenes utilice las opciones
         :kbd:`-passin` y :kbd:`-passout` y consulte la sección "Pass Plhare
         Options" de :manpage:`openssl(1)`.
...
|DER|
   Es un formato de certificado binario (o sea, no imprimible) muy usando en los
   sistemas *Windows*. O contiene la parte privada o la pública, pero no ambas.
   Suele encontrase en archivos con extensión:

   * ``.der``, que generalmente contiene una u otra parte.
   * ``.cer`` o ``.crt``, cuando contiene sólo la parte pública. La forma más
     simple de distinguir si un fichero con esta extensión es |PEM| o |DER| es
     mirar su contenido con :ref:`cat <cat>` y ver si contiene caracteres
     imprimibles o no.
   ...
   #. Trasformar un certificado público |DER| al formato |PEM|::

         # openssl x509 -inform der -in CERTIFICADO.crt -outform pem -out CERTIFICADO.pem

   #. Trasformar la clave pública |DER| al formato |PEM| (y proteger la
      clave con un cifrado |AES|\ 256)::
...
Dento de los datos podemos distinguir:
...
La campos propios de la extensión también pueden obtenerse, aunque hay que
...
Supongamos que tenemos el certificado :file:`CERTIFICADO.pem` del epigrafe
...
Hay algunas variamtes interesantes a esta orden:

* La opción :kbd:`-no-CApath` evita que se usen los certificados raiz instalados
...
se desea comprobar y el certificado firmante.

.. warning:: En el caso del |PEM| con el certificado firmante, cerciórese de que
   no está incluido el propio certificado u obtendrá un :kbd:`Unauthorized (6)`. 

.. rubric:: Notas al pie

.. [#] Gratuito actualmente. Durante mucho el servicio fue de pago y
       restringido.
.. [#] Las órdenes sólo exponen cómo hacer conversiones entre formatos y ver ls
       datos del certificado. Por supuesto, se hay otras operaciones
       interesantes como crear certificados o verificar su validez.
