...
.. note:: También es posible firmar y cifrar mensajes con determinados servicios
   ...
   Chromium_ existe la extensión Mailenvelope_ que permite el cifrado de
   ...
   * ``.cer`` y ``.crt`` cuando contiene sólo la parte pública.
   ...
   #. Generar un archivo ``.p12`` a partir de los archivos ``.pem`` y ``,key``::
      ...
      .. note:: Como se necesita introducir una clave para descifrar el
         ...
         Phrase Options" de :manpage:`openssl(1)`.
...
**Certificado personal**
   Conceptualmente es hacer lo mismo (generar la clave privada, hacer la
   solicitud y generar el certificado con ella), aunque con diferencias::
   ...
   * Aunque podemos usar la suborden :command:`x509` para firmar el
     certificado a partir de la solicitud\ [#]_, utilizamos la suborden
     :command:`ca` que organiza mejor la actividad de nuestra |CA|, pues modifica
     el número de serie según vamos creando los certificados y crea un
     registro con todos los certificados que hemos ido generando (en
...
+ Incluir la firma (o las firmas si se requiere la firma de varios firmantes)
  en un campo interno.
+ Utilizar :ref:`certificados digirales <cert-digital>` y no meramente un par
  de claves.
...
Puede comprobarse la validez de la firma de un |PDF| con :command:`pdfsig`
...
un almacen |NSS| y los certificados del sistema operativo. Se puede no obstamte
preparar un catálogo de certificados que contenga la cadena de certificados tal
como se expone :ref:`al tratar los catálogos NSS <cert-nss>` y hacer entonces la
comprobación::
...
Por lo general, las aplicaciones usan dos tipos de catálogos de certificados:

+ El de OpenSSL_, que es el que usa *Linux*.
+ Los generados con la librería |NSS|\ 3, originaria de Netscape_ y que suelen
...
que contiene cada uno de los certificados confiables (en realidad, enlaces
...
|NSS|
-----
Hay aplicaciones que usan la librería |NSS|\ 3 para la gestión del almacen de
certificados. Tal es el caso de navegadores como Chromium_, Firefox_ o la
orden :manpage:`pdfsig` para el soporte de firma en documentos |PDF|. Este
almacen tiene la forma de una base de datos (modernamente SQLite_). Para la
gestión de estos almacenes podemos instalar el paquete::

   # apt install libnss3-tools

que trae la orden :command:`certutil`. Si queremos crear un almacen de claves
*ex-novo* podemos hacer::

   $ cerutil -d sql:/tmp/nssdb -N --empty-password

La opción :kbd:`--empty-password` deja sin cifrar las claves privadas que puedan
almacenarse. Para ver las claves contenidas::
...
En este caso, hay un certificado de usuarios que se reconoce como tal
(:kbd:`u,u,u`), porque la clave privada está también en la base de datos. Por
otro lado, la |CA| raíz debe tener los atributos :kbd:`C,C,C`.

Si queremos ver la información de un certificado en particular::

   $ cerutil -d sql:/tmp/nssdb -Ln 'PERICO_DE_LOS_PALOTES'

Si tenemos un archivo |PKCS| #12 con toda la cadena de confianza, podemos
importarla con::

   $ pk12util  -d sql:/tmp/nssdb -i micert.p12

aunque al certificado raíz habrá que modificarle los atributos para que sea
confiable::

   $ certutil -d sql:/tmp/nssdb -M -n 'AC RAIZ FNMT-RCM - FNMT-RCM' -t 'C,C,C'
...
certificado raíz, podríamos haber añadido este con la orden::
   ...
   $ certutil -d sql:/tmp/nssdb -A -n 'AC RAIZ FNMT-RCM - FNMT-RCM' -t 'C,C,C' -i raiz.pem

donde :file:`raiz.pem` está en formato |PEM| con codificación Base64_.

.. note:: Es posible, también, que las aplicaciones que usen este catálogo
   consulten los certificados del sistema incluidos en :file:`/etc/ssl/certs`.
   Para ello es necesario tener instalado el paquete `p11-kit-modules
   ...
   De este modo, sólo será necesario el certificado intermedio, y no el raíz ya
   que éste se tomará del catálogo general.
