...
.. note:: Aunque conceptualmente una firma sea un resumen cifrado con clave
   privada, en la práctica es necesario añadir metainformación. Téngase presente
   que se requerirá conocer quién lo ha firmado o cuándo se firmó.
  
.. _firmdig-concepto:

Firmemos haciendo uso de :ref:`openssl <openssl>`. Partamos para ello de que ya
hemos generado un par de claves |RSA| (:file:`privkey.pem`, :file:`pubkey.pem`)
según :ref:`lo descrito en el epígrafe anterior <gen-cla-openssl>` y firmemos::
...
Podremos comprobar la veracidad de la firma usando la clave pública, la firma y
el archivo::
...
.. note:: Cuando lo que se tiene no es un simple par de claves, sino un
   ...
   manuual extrayendo la clave pública del certificaddo::
...
asimétrico <gpg-pgp>`. Lo habitual es que los agentes de correo (|MUA|), como
:ref:`mutt <mutt>` o Thunderbird_, implementen la funcionalidad, aunque también
es posible firmar y cifrar mensajes con determinados servicios de *webmail* y
la extensión adecuada del navegador. Por ejemplo, en Chromium_ existe la
extensión Mailenvelope_ que permite el cifrado de mensajes para los principales
sitios de webmail (Gmail_, Yahoo_, Outlook_, etc.).

.. rubric:: Firma manual

Aunque, ya sea mediante |MUA| o ya sea mediante *webmail*, el acto de firmar es
relativamente sencillo, aprovechemos que en el epígrafe anterior
:ref:`generamos un par de claves PGP <gnupg.1>` para ilustrar cómo se firma con
ellas un archivo::
...
|PEM|
   ...
   * ``.cer`` y ``.crt`` cuando contiene sólo la parte pública.
   ...
   #. Comprobar que una clave privada (en :file:`CERTIFICADO.key`) corresponde
      a un certificado público (en :file:`CERTIFICADO.pem`)::
...
El propósito del libro es generar un certificado de |CA| raíz, un certificado de
|CA| intermedia firmado con el anterior y, finalmente, certificados de usuario y
de servidor firmados con el segundo certificado.
...
Documentos |PDF|
----------------
Según :ref:`se presentó en un principio <firmdig-concepto>`, la firma de un
...
certificados que constituyan la cadena de confianza y que se incluyerán en el
...
.. [#] Firefox_, por ejemplo, trae su propio catálogo de certificados raiz y no
