...
un documento acreditativo (el certificado) para asegurar la identidad de la
clave, Es obvio que el problema de suplantación no se soliciona simplemente por
adjuntar los datos de identificación, sino porque el tercero avala la relación
entre la clave y la identidad firmando el conjunto. La estrategia que se sigue
para lograr la **confianza** del firmante es vital en el proceso y hay dos:
...
* Una **red de confianza** totalmente descentralizada en que las relacionaes de
  ...
  Así, si dos propietarios "A" y "B" están seguros de la veracidad del
  ...
  propietario "C", que no haya entrado en contaco con "A", pero si con "B",
  tendrá una relación de confianza directa con "B" y, en consecuencia,
  también podrá confiar en la veracidad del certificado de "A", puesto que "B"
  lo avala y él confía en "B" (relación de confianza indirecta). Los
  :ref:`certificados PGP <openpgp>`, de los que discutiremos al tratar el
  :ref:`correo electrónico <email-seguro>`, fundamentan su confiabialidad así.

* Una infraestructura |PKI|, que es una estructura jerárquica de confianza. En
  ella el tercero que acredita la identidad del propietario deil certificado es
  una :dfn:`autoridad de certificación` (|CA|). Esta |CA| a su veces gozará de
...
`ca-certificates <https://packages.debian.org/search?keywords=ca-certificates>`_
con certificados raíz que reconoce como confiables.
...
Sea como sea, las |CA| generan certificados:
...
Además, no sólo certifican la identidad sino también la finalidad del propio
certificado, por lo que los datos identificativos incluidos en la firma también
contendrán la finalidad de uso. Por ejemplo, un certificado puede o no estar
habilitado para firmar otros certificados.
...
Por último, cada |CA| suele clasificar los certificados que emite dependiendo
del grado de verificación que haya exigido o del grado de seguridad de las
claves que use. Tales clasificaciones no siguen unas mismas reglas, pero suelen
coincidir en la nomenclatura: "Clase 1", "Clase 2", "Clase 3", etc. donde mayor
es el cardinal cuanto mayor el grado. Por ejemplo, una clasificación podría ser
la siguiente:
...
.. note:: En España la autoridad de certificación que usan muchas
   administraciones públicas y una gran mayoría de particulares es la `Fábrica
   Nacional de Moneda y Timbre <http://www.cert.fnmt.es>`_. gracias a una
   `extensa red de oficinas de registro <http://mapaoficinascert.appspot.com/>`_
   (|RA|) que incluye oficinas de la seguridad social, de la agencia tributaria
   o ayuntamientos.
...
la cadena de confianza a través de las firmas que contienen los certificados de
la cadena. Un certificado puede haberse revocado antes de caducar y, en
consecuencia, dejar de ser válido antes de tiempo. Por eso, las |CA| deben
ofrecer un servicio para la comprobación de estas revocaciones. Por ejemplo, la
|FNMT| ofrece este servicio de forma gratuita\ [#]_ mediante el protocolo
estándar |OCSP| descrito en el :rfc:`6960`.

.. seealso:: Más adelante relizaremos una :ref:`verificación completa de la validez de
   un certficado <verif-cert>`.
...
en el próximo epígrafe), nos toparemos con que::
   ...
   $ openssl x509 -in /etc/ssl/certs/letsencrypt-fullchain.crt \
...
|PEM|
   ...
   * ``.cer`` y ``.crt`` cuando contiene sólo el certificado.
   ...
   #. Generar un archivo ``.p12`` a partir de los archivos ``.pem`` y ``,key``::
      ...
      supuesto que tengamos partes pública y privada en distinto archivo. Si
      están contenidas en el mismo |PEM| basta la opción :kbd:`-in`.
      El archivo con la parte pública debe contener al menos la clave pública
      del certificado, pero también puede contener la de la(s) autoridad(es) de
      certificación. Si el certificado de la autoridad está en archivo
      distinto, pero se quiere incluir dentro del resultado puede referirse
      este tercer archivo mediante la opción :kbd:`-CAfile`.
...
|PKCS| #12 (``.p12`` o, a veces, ``.pfx``)
   ...
   :rfc:`7292` y contiene tanto el certufcado como la clave privada del
   certificado. También puede contener certificados de |CA| intermedias. En este
   formato toda la información contenida esta cifrada con una clave simétrica (a
   diferencia del |PEM| que sólo cifra la clave privada) y es el que suelen usar
   los navegadores para exportar e importar los certificados personales de los
   usuarios.
...
**Certificado intermedio**
   Este certificado debe estar firmado con el certificado anterior::

      $ openssl genrsa -out store/inter.key 4096
      $ openssl req -new -config ssl.conf -key store/inter.key -out store/csr/inter.csr
   ...
   + Cambia el modo en que se genera el certificado (orden tercera) para
     firmarlo con el certificado raíz.

**Certificado personal**
   Se cifra con el certificado intermedio::
   ...
   * Utilizamos la suborden :command:`ca`, en vez de :command:`x509`, porque
     el archivo de configuración está preparado para utilizar el certificado
     intermedio como firmante y con él se irá incrementando automáticamente el
     número de serie (:file:`store/serial`) según se vayan creando nuevos certificados
     y, además, se creará un índice que lista todos los certificados creados
...
simbólicos a donde realmente se encuentran) y un archivo
:file:`ca-certificates.crt` en formato |PEM| que contiene concatenados todos
los certificados disponibles. Adicionalmente, hay un archivo
:file:`/etc/ca-certificates.conf` en que se pueden vetar certificados (échele
un ojo al archivo para saber más sobre ello).
...
Empecemos por crear un almacén de claves *ex-novo*::
   ...
   $ cerutil -d sql:/tmp/nssdb -N --empty-password
...
Si queremos ver la información de un certificado en particular::
   ...
   $ cerutil -d sql:/tmp/nssdb -Ln 'PERICO_DE_LOS_PALOTES'
...
.. note:: Si se desea que las aplicaciones que usen este catálogo |NSS|
   ...
   será necesario el paquete `p11-kit-modules
   <https://packages.debian.org/search?keywords=p11-kit-modules>`_ y rescribir
   el archivo :file:`pkcs11.txt` dentro del directorio de catálogo del siguiente
   modo::
...
.. [#] Aunque no en todos los casos. Por ejemplo, para la instalación de un
   ...
   clientes. Sin embargo, se incluye el *script* para automatizar la generación
   de los certificados con lo que no es necesario utilizar (y conocer)
   :program:`openssl`.
