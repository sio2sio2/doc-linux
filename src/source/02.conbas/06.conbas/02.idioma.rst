...
:kbd:`set-keymap` o :kbd:`set-x11-keymap`. La diferencia es que el primero
también afecta a la configuración de teclado. Por ejemplo::

   $ localectl set-x11-keymap es 105

escoge el teclado de España de 105 teclas. Es posible averiguar la lista de
tclados disponibles mediante diversos subcomandos. Éche un ojo a la
página de manual de :manpage:`localectl`.

.. warning:: Tenga presente que dentro de un entorno de escritorio usa la
   ...
   configuracióno particular para el usuario que prevalece sobre la
...
.. note:: La configuración mediante :ref:`localectl <localectl>` se limita
   internamente a cambiar los archivos que se han mendionado en éste y en el
