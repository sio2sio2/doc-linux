...
Los ficheros del sistema se encuentran protegidos mediante un sistema de
...
En el sistema clásico de permisos de *unix* cada fichero tiene un usuario
...
*permisos UGO* (de **U**\ usuario-\ **G**\ rupo-\ **Otro**\ ), porque consiste
en definir los permisos del fichero para:
...
Tal fichero pertenece a un usuario llamado *syslog* y a un grupo llamado *adm*.
...
Por tanto, para el caso del fichero :file:`/var/log/syslog` los permisos se
...
qué significan exactamente. Dependiendo de si el fichero es un fichero regular o
...
**Fichero regular**
   ...
   * Permiso de *ejecución*: Permite ejecutar el fichero como programa, si este,
...
**Directorio**
   * Permiso de *lectura*: Implica ver el contenido de un directorio, esto es,
     poder comprobar que ficheros y subdirectorios contiene. Por tanto, podremos
     usar sobre él :command:`ls`.
   * Permiso de *escritura*: Implica modificar su contenido lo cual se traduce
     en poder crear y borrar ficheros dentro de él.
...
.. warning::
   Téngase en cuenta que la posibilidad de poder borrar un fichero no reside en
   los permisos del propio fichero, sino en los permisos del directorio en el que
   se encuentra. Si tenemos permisos de escritura sobre el directorio, podremos
   borrar el fichero. En caso contrario, no; aun teniendo permiso de escritura
   sobre el fichero. Lo más que podremos hacer en este último caso es dejarlo
   vacío.

Permisos especiales
"""""""""""""""""""

Además de los permisos habituales, se pueden definir sobre los ficheros otros
tres permisos:
...
ejemplo, la cifra **1750** representa un fichero con el bit pegajoso activo (\ **1**\ ),
...
**Directorios**
   * *Bit pegajoso*: Los ficheros que contiene sólo pueden ser borrados por el
     propietario del propio directorio o el dueño del fichero que se pretende
     borrar. Un caso típico es el del fichero temporal :file:`/tmp`, ya que
     ...
     borren ficheros ajenos. La solución, pues, es fijar el bit pegajoso.
   * *setgid*: Todos lo fichero creados dentro tendrán como grupo
     propietario el grupo propietario del propio directorio y no el grupo. Además,
     en el caso de subdirectorios, estos tendrán también habilitado el *setgid*.
     principal del usuario que crea el fichero (que es lo habitual).
...
.. note::
   Aparecen letras minúsculas cuando el fichero tiene definido el permiso de
...
Cuando se crean fichero y directorios, estos pertenecen al usuario que lo crea
...
:command:`umask`
   El directorio se ha creado con permisos 755 y el fichero con permisos 644. Esto
   es debido a que hay definida en el sistema una máscara predeterminada::

      $ umask
      0022

   Para saber los permisos con los que se creará un fichero es necesario hacer
   ...
   máscara es **0022**, los ficheros se crearán con permisos **0644** y los
   ...
   .. note::
      Obviamente, el cambio sólo tendrá efecto durante la sesión. Si se quiere
      hacer permanente, se deberá incluir el comando en algún fichero de
      :ref:`arranque de sesión <inicio-bash>`.
...
:command:`chgrp`
   Permite cambiar el grupo propietario del fichero. Sólo el propietario del
   fichero (o *root*) es capaz de hacer este cambio y sólo podrá cambiar a un
...
:command:`chmod`
   Permite cambiar los permisos sobre un fichero a su propietario (o al
...
--------------------
**ext4** permite asignar a los ficheros una serie de atributos que modifican
...
:command:`chattr`
   ...
   signo menos para desactivarlo. Por ejemplo, para hacer inmutable el fichero
   ...
   Hecho esto, será imposible alterar de forma alguna el fichero, incluso siendo
   administrador::
...
permiten asociar a un fichero atributos arbitrarios de texto con su respectivo
valor. Para poder utilizarlos es necesario haber instalado el paquete *attr* y
que el sistema de ficheros haya sido montado con la opción user_xattr::
...
.. [#] Para entender bien cómo es esta operación binaria recuérdese que en
       ...
       ``110-110-110 & 111-101-101 = 110-100-100``, esto es, **644**.

       Si no nos queremos calentar la cabeza, simplemente nos vale con restar
       en decimal. Para ficheros obtendremos, 666-022 = **644** y, para
