...
#. Paquetería universal:
   ...
   *software* ya instalado a través de la paquetería propia de la distribución;
   ...
   + Flatpak_, promovida por freedesktop.org_.
   + Snap_., promovida por *Canonical*, la empresa detrás de *Ubuntu*.
   + AppImage_, que no es exactamente igual a las dos anteriores, sino que
     participa del concepto de los archivos ``app`` de MacOS: el archivo
     contiene todas las dependencias necesarias para que se ejecute la
     aplicación y no se instala: al ejecutarlo se monta su contenido y arranca
     el programa. Eliminar la aplicación consiste básicamente en eliminar
     el archivo.

   Las dos que tienen más papeletas para impornerse son las dos primeras y,
   especialmente, la primera que goza de más consenso entre las distribuciones
   de *Linux* que la segunda.
...
#. Instalación manual:
   ...
   + En ocasiones, se nos ofrecen *paquetes* de algún sistema de paquetes
     que utilicemos en nuestro sistema. Esta es la fórmula más sencilla para la
     instalación, aunque puede estar sujeta a incompatibilidades (fallos de
     dependencias como ya entenderemnos) y carecerá de una actualización a
     través del gestor de paquetes.

     .. note:: Cuando se nos ofrece, no un paquete aislado de *software*, sino
        un repositorio del que obtener el paquete, estamos en uno de los dos
        casos anteriores y, por tanto, la instalación no es manual. Para
        entender todo esto deberá completar la :ref:`lectura sobre los sistemas
        de paquetes <conceptos-paquetes>` incluida más abajo.`
   ...
   + Finalmente, también puede darse el caso de que de la aplicación sólo se
     ...
     compilemos leyendo la documentación. Es la instalación más ardua por este
     paso que no siempre es trivial y, además, una vez completado nos
...
separados de los paquetes de de aplicación final a fin de que las dependencias
no se instalen duplicadas. De este modo, los dos paquetes de sendas aplicaciones
finales, que dependan de una misma librería, no requerirán incluir dicha
librería dentro de sí, sino únicamente una dependencia al paquete que instala la
librería. Esto elimina, pues, duplicidades, pero introduce relaciones entre los
distintos paquetes y provoca que el sistema de paquetes evoluciones como un
...
:dfn:`Gestores`
   ...
   *Debian* o :program:`flatpak`, que festiona el sistema homónimo de paquetes
   universales.
...
:dfn:`Multigestores` (terminología propia)
   ...
   conjugar varios sistemas de paquetes (siempre que haya un sólo sistema de
   distribución). Por ello, a los *multigestores* que gestionan varios sistemas
   de paquetes los llamaremos :dfn:`agregadores` (terminología también propia).
   Los multigestores más conocidos son:
   ...
   + PackageKit_, independiente del entorno de escritorio e incluso con interfaz
     ...
     proporciona una :abbr:`API (Application Programming Interface)` paque que
   ...
   la apariencia de una "tenda de aplicaciones" al modo de las que hay en
...
Paquetería propia
=================
.. warning:: Este apartado es el dependiente de la distribución hasta el
