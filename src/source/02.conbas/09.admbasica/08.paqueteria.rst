...
.. warning:: A partir de Buster_, no existe el directorio :file:`/bin`, sino que
...
y, en consecuencia, :ref:`which <which>` devolverá como ruta
   esta segunda localización. Hagáse la prueba con :ref:`cp <cp>` o :ref:`ls
   <ls>` y comprobará que la orden no devuelve el paquete que los contiene
   (:deb:`coreutiils`) ya que dentro de este paquete la ruta de ambos programas
   es :file:`/bin/` (:file:`/bin/cp`, :file:`/bin/ls`) mientras que la consulta
   con :command:`which` devolverá :file:`/usr/bin/cp` y :file:`/usr/bin/ls`,
   respectivamente.
