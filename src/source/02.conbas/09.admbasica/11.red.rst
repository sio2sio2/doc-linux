...
.. note:: `Ubuntu <https://ubuntu.com/>`_, aunque basada en *Debian*, usa
   como herramienta de configuración de red, desde su versión 18, netplan_ y no
   :ref:`ifupdown <ifupdown>`.
...
Configuración
=============
INTRODUCCIÓN A LOS TRES MÉTODOS...
...
:manpage:`systemd-resolved` para la definición de los servidores |DNS|. El
método es común a cualquier distribución que use este *init*, aunque como en el
caso de *Debian*, el servicio puede estar desactivado porque se use un método
alternativo\ [#]_.
...
.. [#] Por ejemplo, *Ubuntu* usa netplan_, aunque sólo
   es un frontend que utiliza :program:`systemd-networkd` por debajo.
