...
Por supuesto, para hacer referencia a ficheros y directorios es indispensable
...
:command:`ls`
   ...
   .. note::
      ...
      de :command:`ls`, sólo hay diferencia cuando el fichero es un :ref:`enlace
   ...
   También se puede usar :command:`ls` con un fichero::

      $ ls /bin/cp
      /bin/cp

   En este caso, se muestra el nombre del fichero con lo que, usado así y si no se
   usan comodines o expandibles, no tiene más utilidad que saber si el fichero
   existe. Una opción que le da sentido a *mirar* un fichero con :command:`ls` es
   ``-l``, que permite obtener cierta información del fichero o directorio que se
   consulte::

      $ ls -l /bin/cp
      -rwxr-xr-x 1 root root 124932 ene 14  2015 /bin/cp

   Para cada fichero que consultemos, :command:`ls` nos devuelve:
   ...
   * El siguiente campo, que es un número, representa el número de
     **referencias** al fichero en el sistema de ficheros. Para entender esto,
     tomemos primero un fichero vacío (que podemos crear con `touch`_)::

      $ touch vacio
      $ ls -l vacio
      -rw-rw-r-- 1 usuario usuario 0 oct 14 10:02 vacio

     Este fichero recién creado ocupará un espacio físico en disco y sólo tiene
     una referencia en el sistema de ficheros: a través del nombre *vacío*\ .
     ...
     Ahora hay dos nombres refiriéndose al mismo espacio. Por tanto, aparece un
     *2*.
     ...
     al referirsee a su directorio padre, se refieren a :file:`directorio`::

      $ ls -d directorio
      drwxrwxr-x 3 usuario usuario 4096 oct 14 12:38 directorio

     En cambio, crear un nuevo fichero dentro del directorio no provoca ninguna
     referencia::
     ...
     * Para *ficheros*, representa el número de enlaces (duros) al propio
       fichero.
   ...
   * El último campo el **nombre** del fichero.
   ...
   Por defecto, :command:`ls` ordena los ficheros alfabéticamente, pero en
...
:command:`stat`
   Además de :kbd:`ls -l` otro modo de obtener información sobre ficheros es el
   ...
   * La fecha de *modificación* que indica el último momento en que se modificó
     el contenido de un fichero.

   * La fecha de *acceso* que indica el último momento en que se accedió (leyó)
     ...
     monte los sistemas de ficheros con la opción *noatime*, para que la fecha
     de acceso sólo se actualice o una vez al día o cuando se modifica el
     fichero.

   * La fecha de *cambio* que indica el último momento en que se cambiaron los
     atributos de un fichero (propietarios, permisos, etc.).

   * La fecha de *creación* que indica el momento en que se creó el fichero. En
   ...
   representa el nombre de propieta y ``%u``, uid del propietario. Así pues::
...
:command:`tree`
   ...
   ``-a``
      Como la opción de :ref:`ls <ls>`, muestra también los ficheros ocultos.
...
:command:`file`
   Por lo general, las extensiones se usan para saber de qué tipo es un fichero,
   pero no tienen por qué haber sido bien elegidas. :command:`file` es un comando
   que intenta averigüar de qué tipo es, basándose en su contenido::

      $ file primera_carlista.mp4
      primera_carlista.mp4: ISO Media, MP4 Base Media v1 [IS0 14496-12:2003]

   Si se usa la opción :kbd:`-i`, devuelve el tipo *mime*::
...
:command:`whereis`
   :command:`ls` puede usarse para comprobar si un fichero se encuentra o no en un
...
:command:`find`
   ...
   fichero cualquiera. Para ello puede usarse el comando :command:`find` que
   ...
   es indistinto y deben colcarse según se han enunciado: primero las de
   ...
   ``-mount``:
      No busca en subdirectorios que pertenezcan a sistemas de ficheros
   ...
   Opciones de **selección**:
      ...
      fichero resulta verdadera, el fichero se seleccionará.

      ``-name <expresion>``:
         permite buscar ficheros por su nombre según la expresión que se indique. Esta
      ...
      ``-path <expresion>``:
         busca ficheros atendiendo a toda su ruta y no sólo al nombre. Existe
         también la variante :kbd:`ipath` para no distinguir maýusculas de
         ...
         ejemplo, si el directorio es :kbd:`.` las rutas de todos los ficheros
         encontraods empezarán por ``./``; si :kbd:`dir`, todas empezarán por
         ``dir/``.
      ...
      ``-type f|d|l|p|b|c|s``:
         restringe la búsqueda al tipo de fichero indicado. El carácter es el mismo
         que el que muestra la opción ``-l`` de ls_, excepto en el caso de los
         ficheros regulares que se usa la :kbd:`f` en vez del guión.`

      ``-samefile <fichero>``:
         busca los otros :ref:`enlaces duros <hardlink>` del fichero indicado.

      ``-empty``:
         el fichero (o directorio) está vacío.

      ``-mtime <n>``:
         el fichero se modificó hace *n* días. Si se hace negativo el número,
      ...
      ``-newer <fichero>``:
         busca los ficheros modificados después del fichero de referencia
         proporcionado como argumento. ``anewer`` y ``cnewer`` sirver para lo
         mismo, pero usando las fechas de acceso y cambio.

      ``-group <grupo>``:
         busca ficheros cuyo grupo propietario sea el indicado (puede usarse el
         nombre o el *gid*).

      ``-user <usuario>``:
         busca ficheros propiedad del usuario indicado (puede usarse el usuario o
         el *uid*).

      ``-links <n>``:
         busca ficheros que tengan *n* enlaces. Pueden usarse los signos *-* y *+*
         con el mismo significado que para :kbd:`mtime`.
         
      ``-size <n><unidad>``:
         busca ficheros de un tamaño determinado. En cuanto a la cantidad pueden
   ...
   Opciones de **operación**:
      ...
      ``-print``:
         imprime en pantalla los ficheros seleccionados, uno por línea. Hay
         variantes de esta opción, pero hay una indispensable en ciertos casos que
         es :kbd:`print0` que imprime de modo que el carácter que separa un fichero
         de otro es el carácter nulo.

      ``-delete``:
         borra el fichero indicado. Si el fichero es un directorio, debe estar
         vacío.

      ``-exec <comando> '{}' +``:
         ejecuta el comando indicado sobre los ficheros seleccionados. El '{}' representa
         tales ficheros que se usaran como argumentos para comando.  Al incluir
         el signo ``+``, no se ejecuta un comando por cada fichero, sino que se
      ...
      ``-ok <command> '{}' \;``:
         ejecuta el comando para cada uno de los ficheros seleccionados, pero
   ...
   #. Buscar todos los ficheros ``.pdf`` bajo el directorio de trabajo::

         $ find -iname '*.pdf'

      Como no se especifica acción alguna, se sobreentiende ``-print``.

   #. Lo mismo, pero especificando que se buscan ficheros regulares y no
      cualquier otra cosa (enlaces simbólicos, por ejemplo)::

         $ find -type f -iname '*.pdf'

   #. Buscar un fichero llamado :file:`deseable.txt` bajo el directorio de
      trabajo y para la búsqueda nada más encontrarlo::

         $ find -type f -name 'deseable.txt' -print -quit

      En este caso, para cada fichero encontrado, primero se imprime y luego se
   ...
   #. Busca bajo el directorio :file:`/home/store/video` los ficheros mayores a
      500MB::

         $ find /home/store/video -type f -size +500M

   #. Busca bajo el directorio actual los ficheros con más de un enlace::
   ...
   En este ejemplo se buscan ficheros que sean regulares (:kbd:`-type f`) **y**
   ...
   #. Busca todos los ficheros que no estén vacíos::
      ...
      que es más immediato. pero hacerlo de otro modo ilustra mejor cómo
      evalúa :command:`find`. En este último caso, ademäs, podemos ahorrarnos
      ``-print``.

   #. Busca ficheros ``.pdf`` bajo el directorio actual, pero desecha los
      ...
      contenidos, por lo que es necesaria la acción ``-prune`` para descartarlo
      por completo. Antes se omitió este ``-prune``, porque al concordar la
      expresión con directorios vacíos, esto implica que no tengan nada dentro.

   #. Busca ficheros regulares y enlaces simbólicos::

         $ find \( -type f -o -type l \) -print

      Es necesario agrupar para que ambas condiciones se apliquen a la misma
      acción ``-print``. No incluir los paréntesis implicaría no hacer nada para
   ...
   .. note::
      :command:`find` puede ser muy útil a la hora de seleccionar ficheros de los
...
:command:`du`
   Sirve para estimar el espacio de disco que ocupan los directorios (o ficheros)
   ...
   opción ``-s`` que devuelve solamente el espacio ocupado por el fichero que se
   le ha dado como argumento. Además dispone de una opción ``-h`` que se
   comporta como la de ``ls``. Por tanto::

      $ du -hs ~
      3,4G  /home/josem

   Nos devolverá el espacio total ocupado por nuestro directorio personal. Es
   útil añadir la opción ``-x`` que no tiene en cuenta los subdirectorios que
   monten otros sistemas de ficheros.

... y dentro de ficheros
~~~~~~~~~~~~~~~~~~~~~~~~
.. _cat:
.. index:: cat

:command:`cat`
   El comando más simple que permite consultar el contenido de un fichero es
   :command:`cat`: simplemente escupe por pantalla, carácter a carácter, el
   contenido. Si el fichero es *texto plano* podremos leerlo, si es un fichero
   ...
   Suponiendo que hubiéramos escrito previamente un fichero que contuviera
   únicamente la palabra *Hola*. Si se escriben dos o más ficheros como argumento,
   ...
   Es posible también usarlo sin indicar fichero alguno: en este caso recibirá los
   datos a través de la entrada estándar (véase :ref:`redirecciones <ioredirect>`).

   De entre sus opciones, es útil a veces ``-n``, que numera las líneas al
   mostrarlas.
...
:command:`more`
   Sin embargo, :command:`cat` tiene el inconveniente de mostrar el fichero de una
   ...
   Al llenar la pantalla, el programa quedará esperando a que pulsemos :kbd:`Enter`
   (avancar una línea) o :kbd:`Espacio` (avanzar una pantalla). También puede
   escribirse una barra (:kbd:`/`) seguida de una :ref:`expresión regular <regex>`
   para buscar la primera ocurrencia que concuerde con dicha expresión,

   Se saldrá del programa al acabarse el fichero o al pulsarse :kbd:`q`.

.. _less:
.. index:: less

:command:`less`
   Como con :command:`more` sólo podemos avanzar, pero no retroceder, existe
   `less`. Tiene un uso semejante, con la diferencia de que hay teclas para ir
   hacia atrás. Consúltese la página del manual.
   ...
   * :command:`bzcat`, :command:`bzmore` y :command:`bzless` para comprimidos con
     :program:`bzip2` (extensión ``.bz`` o ``.bzip2``).
   ...
   * :command:`xzcat`, :command:`xzmore` y :command:`xzless` para comprimidos con
     :program:`xz` (extensión ``.xz``).

.. _head:
.. index:: head

:command:`head`
   Cuando no se quiere inspeccionar todo el fichero sino sólo su comienzo, es útil
   el comando :command:`head`, que permite indicar cuáles de las primeras líneas de
   un texto se desean mostrar::

      $ head /etc/passwd

   Si no se espècifica nada, se muestran 10 líneas. Si se quiere mostrar un número
   distinto es posible hacerlo con la opción ``-n``::

      $ head -n20 /etc/passwd

   El ejemplo, mostrará 20 líneas. Es posible también mostrar todas las líneas
   menos las *n* últimas, haciendo negativo el argumento de ``-n``::

      $ head -n-1 /etc/passwd

   Esto mostraría todas las líneas menos la última.

   Como con los comandos anteriores, si no se indica nombre alguno de fichero, se
...
:command:`tail`
   ...
   últimas líneas del fichero. Si no se especifica cuántas::
   ...
   :command:`tail`, además, admite la opción ``-f``, que es muy útil cuando se
   quiere monitorizar porque muestra las últimas 10 líneas del fichero (o cualquier
   otro número si se usa además ``-n``) y queda esperando a que el fichero se
...
:command:`watch`
   No es un comando propiamente para ver el contenido de un fichero, sino para
   refrescar cada cierto tiempo (por defecto cada dos segundos) la salida por
   pantalla de una orden. Así si hacemos::

      $ watch -n1 "cat /proc/mdstat"

   nos mostrará el contenido del fichero :file:`/proc/mdstat` (ya que eso es lo
   que hace :ref:`cat <cat>`) e ira refrescando ese contenido cada segundo. Por
   la misma razón::
...
:command:`mkdir`
   La acción más sencilla para alterar nuestro sistema de ficheros es crear un
   nuevo directorio::
   ...
   la opción ``-p`` que crear un directorio, crenado previamente todos los directorios
   necesarios::
...
:command:`touch`
   En general, para crear un fichero de un determinado tipo lo que debe hacerse es usar una
...
:command:`rm`
   Si, en vez de crear un fichero, queremos borrarlo, podemos usar el comando
   ...
   Como por error podemos borrar un fichero sin querer, :command:`rm` proporciona
   ...
   Además, el comando sirve en principio para borrar ficheros, por lo que si se
...
:command:`mv`
   Otra operación habitual con ficheros (o directorios) es moverlos de lugar, para
   lo cual existe el comando :command:`mv`. La sintaxis general es la siguiente::

      $ mv [opciones] <origen> <destino>

   Es decir, el fichero se mueve desde un origen a un destino. El origen es el
   ...
   es un fichero, sobrescribimos::

      $ mv dir2/fic.renombrado fic2
      ls dir1 dir2 fic2

   Si tomaramos como origen un directorio en vez de un fichero las consecuencias
   ...
   Para que la orden funcione el destino debe ser un directorio y existi; así lo
   ...
   ========= ========== =======================================================
   F         F          El fichero se mueve al destino. Si este existe,
                        sobreescribe; si no, cambia de nombre.
   F         D          El fichero se mueve dentro del directorio.
   D         F          **Error**\ .
   D         D          El origen se mueve dentro del directorio de destino. Si
                        no existe, cambia el nombre.
   ========= ========== =======================================================
...
:command:`cp`
   ...
   Cuando movemos un fichero, no cambia sus propiedades (fechas, permisos,
   ...
   En principio, :file:`cp` copia ficheros, pero es posible copiar también
   ...
   sistema de ficheros. Dentro de este sistema de ficheros, se encuentra un fichero
   que se encuentra referencia a través de dos nombres (*nombre1* y *nombre2*). Al
   crear un enlace duro lo que estamos haciendo es crear una nueva referencia a
   partir de una que ya existía. En el sistema de ficheros, no hay dos ficheros,
   sino un único fichero con dos nombres. La limitación es que ambas referencias
   sólo pueden existir dentro del mismo sistema de ficheros. Por ese motivo, si
   intentamos crear un enlace duro en otro sistema de ficheros, obtendremos un
   error. Supongamos que :file:`/home` y :file:`/tmp` están en distinto sistema de
   ficheros::
   ...
   nombre es *más importante que otro*: el fichero se borrará definitivamente,
...
:command:`ln`
   ...
   fichero. En el dibujo se expresa esta idea haciendo que *simbólico* señale a
   *nombre1*. Para crearlos puede usarse la opción ``-s``::
   ...
   Hay varias diferencias con el otro tipo de enlace:
      ...
      #. Si se borra el fichero original, el enlace simbólico permanecerá, pero
         apuntando a algo que ya no existe y, por tanto, deja de tener sentido::
   ...
   .. warning::
      Ha de tenerse cuidado al crear enlaces simbólicos cuando se usa una
      ...
      simbólico. Retomando el ejemplo anterior::
      ...
      Como se quiere usar una ruta relativa, :file:`nombre1` debe extresarse
      respecto al directorio donde se desea crear :file:`simbólico`. Por tanto la
      ruta relativa consiste en escribir simplemente el nombre, aunque nuestro
      dikrectorio de trabajo sea :file:`/tmp` y no el directorio personal.

   .. note::
      Aunque sólo se pueden crear enlaces duros con ficheros, es lícito con
      :command:`cp` copiar directorios con todo su contenido, usar la opción
      ``-l``. En ese caso, :command:`cp` creara un nuevo directorio al copiar
      el directorio y enlaces duros al copiar los ficheros.
...
:command:`readlink`
   ...
   Si se usa como argumento un fichero que no es un enlace simbólico, no
   ...
   ``-f`` que, además, siempre muestra la ruta absoluta::
   ...
   En este caso, tiene sentido usar como argumento cualquier fichero: no se
...
Cuando se usa una interfaz de texto, gran parte de los ficheros que se manejan
...
.. include:: ../../ejercicios/02-copias.rst
