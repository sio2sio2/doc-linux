...
.. note:: Obviamente, si quisiéramos crear un segundo grupo de máquinas en un
   red distinta, bastaría con crear una interfaz puente distinta y asociar tales
   máquinas a ella. (p.e. *vmnet1*).
...
interfaz física del anfitrión de un modo especial. Simplemente, al arrancar la
máquina deberemos preparar una interfaz derivada del siguiente modo::
...
.. note::  Hemos supuesto que hemos creado un grup *qemusers* donde se
   encontrarán todos usuarios que usen :program:`QEmu` tal como sugerimos
...
Preparado el sistema, podremos arrancar la máquina de este modo::
   ...
   $ qemu-system-vga -hda caca.qcw -device virtio-net,netdev=nic,mac=52:54:00:12:34:56 \
        -netdev tap,id=nic,fd=3 3<>/dev/tap$(cat "/sys/class/net/macvtap0/ifindex")

donde puede observarse que la |MAC| de la interfaz macvtap debe coincidir con la
|MAC| que se asigne a la interfaz virtualizada del anfitrión y, además, se usa
...
De este modo, el anfitrión se comunicará a través de la interfaz macvtap
*ethost*, no eth0, y, por tanto, no quedará aislada de los huéspedes.
