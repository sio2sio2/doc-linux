...
.. warning:: Para que en un huésped *Linux* pueda llevarse a cabo la instalación
   ...
   cabeceras del núcleo y también las órdenes :command:`make` y :command:`gcc`,
   por lo que antes de llevar la instalación quizás necesite hacer::
...
#. Se podrá compartir archivos fácilmente entre anfitrión y huésped con
   ...
   .. note:: Si en un huésped con sistema *Linux* desea poder leer y escribir en
      la carpeta compartida con un usuario distinto a *root*, tal usuario deberá
      pertenecer al grupo *vboxfs*.

   .. note:: En un huésped con un sistema *Linux* muy básico en el que no se
      monten automáticamente sistemas de archivos, deberá hacer montar el recurso
      compartido a mano::
