...
* Copiar el directorio que contiene la máquina virtual al sistema de destino y
  crear una nueva máquina virtual a través del archivo :file:`.vbox`  que define
  la máquina y está contenido en ese mismo directorio. El problema de este
  método es que debemos comprobar si todo lo que necesitamos está contenido
  dentro del directorio. Por ejemplo, si utilizamos un disco en *multiconexión*,
