...
.. note:: Un aspecto importante de estas transacciones, aunque no estrictamente
   necesario para entender el concepto, es el de |UTXO| (o transacción de salida
   no gastada). Una :dfn:`UTXO` es una cantidad *indivisible* de criptomoneda que
   se envía a una dirección (por eso es una salida) para que pueda ser gastada
   por el destinatario en el futuro. Por ejemplo, el *Bloque* **1** crea una
   |UTXO| de 5\ |btc| para el señor **A**. ¿Qué significa que es indivisible?
   Significa que en el *Bloque* **2**, en que se transfieren 3\ |btc| del total
   de cinco, no se toma como entrada parte de esta |UTXO|. Es necesario gastarla
   por completo y, como consecuencia, la transacción que hemos reflejado en el
   *Bloque* **2** no es tan simple, sino que en realidad supone:
   ...
   * La creación de una |UTXO| de 2\ |btc| para el propio señor **A**.
   ...
   Algo parecido ocurre con las transacciones de dinero físico. Si quieres pagar
   3€ a alguien y lo haces con un billete de 5€, entregas tal billete y recibes
   una moneda de 2€\ [#]_.

   Por supuesto, se puede crear una |UTXO| a partir de varias |UTXO| distintas,
   incluso de distinto propietario. Por ejemplo, si en el próximo *Bloque* **5**
   el señor **A** quiere transferir sus tres *bitcoins* al señor **B** entonces
   generará una |UTXO| de salida a partir de dos |UTXO| de entrada: la que se
   generó en el *Bloque* **2** como resultado del cambio y la que se generó en
   el *Bloque* **4**. Es muy interesante la lectura del artículo `Transacciones
   en Bitcoin
   <https://sheinix.medium.com/transacciones-en-bitcoin-841a087ff439>`_.
...
**Distinción de clientes**
   ...
   representado por su *wallet*, de manera que cuando se hacen transacciones se
   hacen transferencias a los *wallets*. Sin embargo, a diferencia de lo que
   podría parecer por su nombre y la analogía con el mundo físico, los *wallets*
   no contienen monedas, sino la clave privada asociada al cliente, a partir de
   la cual se obtiene la clave pública correspondiente. Usando está clave
   pública y aplicando algoritmos de resumen (véase :ref:`claves de bitcoin
   <prac-claves>`) se calcula la dirección del *wallet*. que es el dato que
   debemos proporcionar a aquel que queremos que nos transfiera criptomoneda. El
   *wallet* es personal e intransferible y, mientras no nos lo roben, podemos
   estar seguros de que dispondremos de las monedas que la cadena de bloques
   determina que son nuestras. Más adelante, :ref:`profundizaremos en el
   conocimiento de estas billeteras <wallets>`.
...
**Inalterabilidad**
   ...
   bloque adecuado antes de poder añadirse a la cadena, del mismo modo que un
...
**Incentivos**
   ...
   + Por bloque minado. Las nuevas monedas se crean al minar bloques y dar esa
     ...
     *bitcoin* como su creador decidió que su cantidad total estuviera limitada
     ...
     mitad. A este hecho de división se lo conoce como :dfn:`halving`.
...
:dfn:`Security token` (:dfn:`token de seguridad`)
   ...
   + :dfn:`Asset-backend token` (*token de activos*), que representan la
     ...
     activo puede ser de muy diversa naturaleza: propiedad inmobiliarias,
     materias primas, etc.
...
**Centralizado** (|CEX|)
   ...
   criptomoneda que se desea vender. Por tanto, estamos cediendo la titularidad
   digital de nuestras criptomonedas (o de parte de ellas) al *exchange*, que
   las apuntará como nuestras en una cuenta. Como durante el tiempo que
   mantengamos las criptomonedas en esta cuenta, la titularidad digital de éstas
   es de la entidad, perdemos el control sobre ellas y estarán expuestas a
   ataques al *exchange* o la quiebra de la entidad. En este tipo de casas de
   cambio pueden intercambiarse criptomonedas por dinero *fiat* o criptomonedas
   entre sí.
...
claves, en cuyo caso podría gastar todas las |UTXO| dirigidas a ese *wallet* y
enviarlas a un *wallet* distinto del que él sea propietario.
...
:dfn:`Billeteras frías` (*cold wallets*)
   ...
   #. :dfn:`Billeteras en papel`
      ...
      escribirlas y leerlas manualmente se presta a provocar errores de
