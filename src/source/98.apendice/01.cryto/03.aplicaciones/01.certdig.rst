...
basta con saber que es el propietario de la clave, sino que además se necesita
asociarlo a una identidad real (p.e. una persona con nombre y apellidos). La
información de quién es nos la puede proporcionar el firmante directamente, pero
¿cómo podemos estar seguros de que no nos miente? ¿Qué garantías tenemos de que
ese propietario es quien dice ser?  Para subsanar este último escollo existen
...
tercero firmante. Pero ¿cómo confiamos en el tercero?
...
|PKCS| #12 (``.p12`` o, frecuentemente en *Windows*, ``.pfx``)
