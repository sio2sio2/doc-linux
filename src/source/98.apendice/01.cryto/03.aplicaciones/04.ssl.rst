...
.. [#] Aunque esta argucia, a diferencia de la que se usó en la parte servidor,
   no es una solución general. Con otros clientes habría que estudiar otra
   triquiñuela distinta (p.e. con :ref:`ssh <ssh>` puede usarse
   :ref:`ProxyCommand <ssh-https>`).
