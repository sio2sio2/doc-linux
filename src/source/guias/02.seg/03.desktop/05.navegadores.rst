...
**Cookies de rastro**
   ...
   **Duración**
      ...
      - Si el tiempo es exageradamente largo (p.e. de años), entonces ls *cookie*
   ...
   **Dominio**
      ...
      - Si especifican dominio y es distinto al del servidor que la envía, el
        ...
        *cookie*, que es distinto al que la envío: son los que se llaman
        :dfn:`cookies de terceros`. Estas *cookies* porque entregan a un segundo
...
**Supercookies**
   ...
   conexión siempre será segura aunque el usuario intente una conexión segura.
   Además, esta información no se elimina cuando se borra el historial de
   navegación.
   ...
   + Un sitio *web* habilita 31 subdominios: "*sub*" y de "*sub-0*" hasta
     "*sub-30*".
   + Todos esos subdominios se caracterizan por enviar la cabecera |HSTS| cuando
     se accede a ellos por el protocolo seguro, pero no por el inseguro.
   + Al acceder el usuario al sitio, se insta a hacer una petición no segura al
     subdominio "*sub*". Si el navegador no la convierte en segura, esa es la
     primera vez que se accede al sitio.
   + Cuando el navegador accede por primera vez:

     * Se hace una petición segura a "*sub*", para que el navegador reciba la
       cabecera |HSTS| y en posteriores visitas se sepa que esa visita no es la
       primera.
   ...
   + Cuando el navegador accede las restantes veces:
     ...
     * Se genera el número binario de 30 dígitos con las treinta respuestas, que
       ...
       primera vez que entró. Ya hemos identificado al cliente.
...
**Huella digital**
   ...
   alacenada por éste) que es requerida en una conexión posterior. Esta técnica,
   en cambio, es pasiva puesto que no se expida ningún documento: al conectarse
   el cliente se le "toman las huellas dactilares" y se comprueba si tales
   huellas coinciden con alguna de la base de datos. Si coinciden, ya tenemos
   individualizado al cliente ; si no coinciden, apuntamos tal huella para el
   futuro.
   ...
   .. seealso:: Es interesane el completo `AmIUnique
...
datos de autocompletato y caché), los navegadores proporcionan en su
...
+ Usar el gestor de contraseñas, sabiendo que:

  * Firefox_ permite en su configuración establecer una contraseña maestra sin
    la cual no se autocompletan las contraseñas almacenadas.
  * Chromium_ no tiene contraseña maestra, y en *Windows* no hay forma de
    protegerlas, pero en *Linux* el navegador es capaz de utilizar
    gnome-keyring_ o kwallet_\ [#]_.

    .. note:: Chromium_ debe ser capaz de identificar si el escritorio usa como
       :ref:`gestor de contraseñas <gnome-keyring-ssh>` uno u otro y
       utilizarlos. En cualquier caso, existe la opción :kbd:`--password-store`
       que permite indicar qué es lo que se quiere usar.
...
+ Puede optarse por utilizar gestores de contraseñas externos a través de
  ...
  ordenador (claro que, en este caso, tendremos que fiarnos de los creadores de
  los complementos)
...
`Modo incógnito`_
   ...
   consiste en que ninguna información de navegación (las cinco categorias
...
`Cookie Autodelete`_
   ...
   por lo que si no se comparte el nevegador, es una buena alternativa.
...
NoScript_
   Bloquea todo el código *Javascript* y sólo permite aquel de los
   sitios incluidos en su lista blanca. La seguridad es mayúscula, pero
   actualmente casi todos los sitios web requieren *Javascript* para funcionar,
   así que no resulta muy práctica y casi es mejor no instalarla a menos que
   entremos en modo paranoico.
...
LocalCDN_
   Muchas librerias de *Javascript*, iconos u hojas de estilo |CSS| se almacenan
...
:ref:`Un servicio VPN <vpn>`
   Trataramos los fundamentos en :ref:`la unidad sobre Redes y servidores
...
.. [#] También se referencia como *del dispositivo* o *del navegador*.
   ...
   ofrece es alguna propia del sistema y alguna propoa de la aplicación que
...
.. |ISP| replace:: :abbr:`ISP (Intenet Service Provider)`
