...
.. https://eltamiz.com/elcedazo/2012/12/16/computador-magico-x-representacion-binaria-ii-enteros/
...
La :dfn:`notación en coma flotante` consiste en utilizar en adjuntar a la
expresión del número una potencia de diez que exprese su magnitud. Por ejemplo,
-56783.3456 podríamos expresarlo como :math:`-56.783456*10^3`. Si, además,
indicamos la potencia de diez de suerte que la parte entera sólo tenga una
cifra, entonces la notación esta *normalizada*. De este modo los ejemplos
anteriores en esta notación normalizada se expresarían como :math:`1.234*10^0`,
...
* El **significado**, esto es, el número significativo. La coma siempre está en
  la misma posición, así que podríamos prescindir de usarla. A este número se
  le suele denominar :dfn:`mantisa` (lo cual aparentemente es incorrecto) ya
  veremos por qué.
* El **exponente**.
...
Esta es, conceptualmente, la explicación de la notación en coma flotante. Sin
...
Obsérvese que hemos añadido a la mantisa los ceros a la derecha y no a la
izquierda como comúnmente se hace. Esto se debe a que el punto *decimal* siempre
estará  entre el primer uno y el segundo, por lo que los ceros a la derecha, en
realidad. son ceros de relleno. Por otro lado, la notación siempre está
normalizada, por lo que el primer dígito de la mantisa si usamos binario siempre
será cero. Por tanto, ¿para qué escribirlo y perder un *bit*? En consecuencia,
se prescinde del **1** que tiene la parte entera y sólo se escribe la parte
decimal, esto es, propiamente la *mantisa*. Por ese motivo, al tercer componente
se le llama mantisa y no significado\ [#]_. La representación quedaría
finalmente así:
...
Sólo queda resolver una cuestión, ¿cómo se expresan los exponentes negativos?
Fácil: usando el complemento a dos.
...
.. [#] Aunque bien es cierto que cuando explicamos la notación en base decimal
   nunca debimos usar el término mantisa, porque aquí no se puede prescindir de
   la parte entera.
