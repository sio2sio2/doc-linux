...
más frecuente), la cabecera |HTTP| está cifrada y los proxies se vuelven
inútiles. Una solución es utilizar un cortafuegos, si el sitio web sólo es
accesible a través de una |IP|. Pero si el sitio tiene varias |IP|, entonces lo
aconsejable es hacer este filtrado por nombre. Como el tráfico web está cifrado,
la única forma de saber que alguien está intentando acceder al sitio, es
a través de la petición |DNS| que intenta hacer la resolución\ [#]_ previa.
