...
manejan los sistemas infomáticos ha ido aumentando. Esto ha atraído el interés
...
anda a aquella que existía en los años 70 cuando el tráfico en internet se
...
Conceptos
=========
Antes de entrar en harina es conveniente definir una serie de términos muy
comunes al hablar sobre la seguridad en los sistemas informáticos:

**Seguridad informática**
   Son las medidas y controles que aseguran la *confidencialidad*, *integridad*
   y *disponibilidad* de todos los activos del sistema de información.

**Activo**
   Cpmponente del sistema informático susceptible de ser protegido. Puede ser
   tanto *hardware*, como *software* como datos.
...
**Amenaza**
   ...
   * **Físicas** o **lógicas**, si atendemos al *activo* del sistema al que
     afectan. Las :ref:`físicas <seguridad-f>` actúan sobre el *hardware*,
     mientras que las *lógicas*, al *software* y *datos*.
...
**Atacante**
   ...
   * :dfn:`Hacker` es un experto en asuntos informáticos, use como use sus
     ...
     o `Richard Stallman <https://es.wikipedia.org/wiki/Dennis_Ritchie>`_ entran
     dentro de esta definición.

   * :dfn:`Cracker` es la persona que vulnera la seguridad de un sistema
     informática de forma ilícita. Si lo hiciera, en cambio, para poner a
...
**Contramedida**
   Es la acción encaminada a prevenir la amenaza que aprovecha cierta
   vulnerabilidad.
...
**Analisis forense**
   ...
   fin de conocer las causas por kas que se ha producido un accidente o las
   vulnerablidades que ha explotado o intendo explotar un ataque.
...
Pese a todo el esfuerzo que se dedique a la seguridad, es indispensable tener
presente que la seguridad absoluta no existe. De hecho, es célebre al respecto
la frase del experto en seguridad y profesor universitario Gene Spafford:
...
* **Disponibilidad**, que consiste en garantizar en todo momento el acceso sobre
  el sistema y sus datos a las persona autorizadas.

* **No repudio**, esto es, garantizar que el emisor de un mensaje no pueda negar
  que lo envió (:dfn:`repudio en origen`), así como que el receptor no pueda
  negar que lo recibió (:dfn:`repudio en destino`).

* **Auditabilidad**, que es la posibilidad de un sistema para ser analizado
  posteriormente mediante el registro de su comportamiento.
...
* La **alteración del software** puede deberse:
  ...
  - Una alteración *manual* llevada a cabo por alguien que ha accedido al
    sistema. Por ejemplo, si logra permisos de administrador, puede susutituir
...
**Software malicioso**
   ...
   También es conocido por el anglicismo :dfn:`malware`. Dependiendo de cómo
   funcionen reciben distinto nombre:

   **Virus**
      Código que se adhiere a un fichero ejecutable y entra en funcionamiento al
      ejecutar éste.
   ...
   **Gusano**
      Código que se propaga de ordenador a ordenador sin necesidad de intervención
      humana. Con la generalización de la conexión de redes de los ordenadores, los
      virus han perdido protagonismo frente a los gusanos.

   **Troyano**
      Es un código malicioso que finge ser un programa legítimo. El troyano más
      famoso del mundo posiblemente haya sido `I love you
      <https://blogthinkbig.com/virus-informatico-i-love-you>`_ que apareció en el
      año 2000 e infectaba el ordenador gracias a un mensaje de correo con un
      prometedor asunto como "I love you" y un adjunto que prometía ser una carta
      de amor. Al pinchar sobre la carta, se desencadenaba su efecto. El troyano,
      además, tenía una componente de gusano ya que su código, además de afectar al
      ordenador, consultaba la lista de contactos y reenviaba correos igualmente
      amorosos a las direcciones de la lista.

   Los daños que ocasionan puede ser diversos:

   + Colapsar el sistema agotando recursos.
   + Generar publicidad.
   + Borrar información.
   + Cifrar la información para cobrar un rescate por ella. Este tipo de
     *malware* recibe el nombre de *ransomware*\ [#]_ y ha florecido en los
     últimos años. El más celebre de ellos fue `WannaCry
...
se produce la amenaza con el objeto de prevenirla. De este modo, son medidas de
securidad activa:

* Todas las medidas de **control de acceso** encaminadas a salvarguardar
  *sotfware*, datos y *hardware*:
...
Seguridad pasiva
----------------
Las técnicas de seguridad **pasiva** se establecen para paliar los efectos
negativos que haya producido una amenaza.

* *Cifrado* que, tras un robo de información, evita que esta se pueda
  leer.

  .. note:: Es obvio que si establecemos que la amenaza no es el mero robo
     de los datos, sino el acceso efectivo a ellos; el cifrado es entonces
     seguridad *activa*, no *pasiva*.
...
* |SAI| ante cortes de suministro eléctrico que mantienen activo el sistema, al
  ...
  que el sistema de apague ordenadamente.
...
* La *redundancia* que permite mantener el servicio ante la pérdida de uno de los
  ...
  almecenamiento. También puede ser redjundante un servicio de correo y cuando
...
Disponibilidad
**************
La :dfn:`disponibilidad` es el grado de garantía que proporciona un sistema
informático de ofrecer su servicio a los usuarios autorizados. Lo ideal es que
la disponibilidad fuera absoluta, pero en la práctica es inevitable que se
...
disponibilidad mínima que se compromenten a ofrecer para el servicio.
...
En cualquier caso, para el cálculo la disponibilidad se definen otros cuatro
conceptos:
...
.. include:: /10.guias/02.seg/99.ejercicios/01.intro.rst
