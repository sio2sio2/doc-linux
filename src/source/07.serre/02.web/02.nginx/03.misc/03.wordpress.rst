...
Se necesitan, adicionalmente, los siguientes módulos de |PHP|::
   ...
   # apt-get install php-{gd,mbstring,mcrypt,xml{,rpc}}
...
.. warning:: Sobre esta configuración es indispensable hacer dos precisiones:
   ...
   + El *plugin* se comporta bien con las páginas del panel de administración,
...
`Invisible reCaptcha <https://es.wordpress.org/plugins/invisible-recaptcha/>`_
   Habilita la `versión 3 de Google reCaptcha
   <https://www.google.com/recaptcha/intro/v3.html>`_. Es necesario generar en
   el enlace facilitado un par de claves con una cuenta de *Google* para poder
   usarlo, ya que la configuración lo requiere. Con las extensión podemos
   proteger el formulario de acceso, el de registro, la creación de comentarios
   y la petición para cambio de contraseña.
...
como `Google Apps Login <https://es.wordpress.org/plugins/google-apps-login/>`_.
...
+ La autenticación con *Google* permite la autenticación alternativa de
  *usuarios* ya registrados en el sitio con, obviamente, una cuenta de
  *Google*. No tiene más ventaja que la de que el usuario no tenga que recordar
  el nombre y la contraseña con que está registrado en nuestro *Wordpress*.

.. warning:: El *plugin* sugerido es incompatible con `invisible reCaptcha
   <https://es.wordpress.org/plugins/invisible-recaptcha/>`_, por lo que en caso
   de usarse, deberá sustituirse éste por otro *plugin* alternativo como
   `simple Google reCaptcha
   <https://es.wordpress.org/plugins/simple-google-recaptcha/>`_.
