...
+ Con `Let's Encrypt`_, puesto que no requiere ningún tipo de registro.
...
generar un certificado para esos nombres se lleva a cabo con::
   ...
   # acme.sh --issue --server letsencrypt -d example.net -d www.example.net -w /srv/www/default

donde :kbd:`-d` introduce los nombres de máquina y :kbd:`-w` el directorio raíz
para ese servidor virtual. La opción :kbd:`--server` indica cuál es la |CA| con
la que queremos generar certificados (véase `todas las alternativas en la
documentación oficial
<https://github.com/acmesh-official/acme.sh/wiki/Server>`_) y es necesario
incluirla en las operaciones que requieren conexión con la |CA| como
...
.. note:: Podemos en un mismo certificado asociar varios nombres a distintos
   ...
   inmediatamente después de cada nombre::
      ...
      # acme.sh --issue --server letsencrypt
                        -d example.net -w /srv/www/default \
...
nuestro servidor :program:`nginx`\ [#]_::
...
Nos pueden interesar algunas órdenes adicionales, como la que nos permite
...
o la que permite revocar y borrar alguno de los certificados::
   ...
   # acme.sh --revoke --server letsencrypt -d example.net
...
.. note:: A partir de la versión **3.0**, `Let's Encrypt`_ dejó de ser la |CA|
   predeterminada y pasó a serlo ZeroSSL_. Esa es la razón por la que hemos
   necesita explicitar el servidor a través de la opción :kbd:`--server`.
   Podemos cambiar, no obstante, muy fácilmente la |CA| predeterminada::
