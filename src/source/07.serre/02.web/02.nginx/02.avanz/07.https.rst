...
identificación del propio servidor\ [#]_, es indispensable que una autoridad
de certificación ampliamente reconocida lo firme. Eso, por lo general, supone:
...
certiificados válidos y es absolutamente injustificable haber generados
...
Otro aspecto a tener en cuenta, ademas de quién nos proporcione el certificado,
es cómo se acreadita ante el certificador la propiedad del dominio. Los métodos
...
#. Mediante **correo electrónico**, que consiste en que la autoridad nos envía un
   ...
   del dominio que hayamos afirmado que es nuestro, como ``admin@example.com``
   o ``webmaster@example.com``. Los nombres de las cuentas son tales que sólo
...
#. Mediante el protocolo |ACME|, inicialmente propuesto como estándar por `Let's
   ...
   :dfn:`Desafío HTTP`
      Consiste en que el servidor entrega al cliente un *token*, para que este
      lo haga accesible mediante un archivo que comporte del servidor web. Para
      conectar con este servidor web la autoridad utiliza el nombre de máquina
      que se pretende acreditar (p.e. ``www.example.com``) y, si lo obtiene,
      acredita entonces la propiedad sobre dicho nombre y genera y entrega un
      certificado válido al cliente.
   ...
   :dfn:`Desafío DNS`
      ...
      certificados comodín (o sea, un certificado para ``*.example.com`` que servirá
      para cualquier nombre de máquina dentro del dominio).

   .. seealso:: Para más información consulte las `explicaciones más prolijas
      de Let's Encrypt sobnre desafíos <acme.sh_0.1-2_amd64.deb>`_
...
+ Está enteramente escrito en *POSIX shell*, lo que evita la necesidad de
  ...
  instaladas.
+ Soporta las tres autoridades que permiten obtener certificados gratuitos.
...
.. warning:: Al desintalar el paquete parace que ni se elimina el trabajo de
   cron (hágase :kbd:`crontab -l` como *root*) ni se elimina la línea que se
   creo en :file:`/root/.bashrc` por lo que habrá que hacerlo a mano.
...
+ El :ref:`desafío HTTP <desafios-acme>`.
  ...
  .. warning:: No obstante, si pretende albergar múltiples sitios en el mismo
     ...
     con el desafio DNS <dns-challenge>`.
...
generar un certificado para esos nombres se lleva a cabo con::
   ...
   # acme.sh --issue -d example.com -d www.example.com -w /srv/www/default

donde :kbd:`-d` introduce los nombres de máquina y :kbd:`-w` el directorio raíz
para ese servidor virtual. 

.. note:: Podemos en un mismo certificado asociar vartios nombres a distintos
   ...
   incluyan antes de su correspondiente directorio raíz::
      ...
      # acme.sh --issue -d example.com -d www.example.com -w /srv/www/default \
                        -d moodle.example.com -w /srv/www/moodle

Una vez generado el certificado, debemos instalarlo::

   # acme.sh --install-cert -d example.com --key-file /etc/ssl/private/letsencrypt-example.com.key \
      --fullchain-file /etc/ssl/certs/letsencrypt-fullchain-example.com.cer
...
no se borrará del directorio :file:`/etc/acme.sh/cert/example.com`. Esta operación deberemos hacerla a mano.
...
en esta ocasión sí tendremos que crear tiene el siguiente contenido) tiene el
siguiente contenido:

.. code-block:: nginx

   ssl_certificate /etc/ssl/certs/letsencrypt-fullchain-example.com.cer
   ssl_certificate_key /etc/ssl/private/letsencrypt-example.com.key
   ssl_trusted_certificate /etc/ssl/certs/letsencrypt-fullchain-example.com.cer
...
limitación el protocolo |SSL| idió la :ref:`extensión SNI <sni>`, que soportan
...
registro previo que puede hacerse desde la propia línea de órdenes::
   ...
   # acme.sh  --server zerossl --register-account -m micorreo@example.com
...
Desafío |DNS|
-------------
Usar este desafío tiene la ventaja de que nos permite crear un certificado
comodín para el dominio. La dificultad se encuentra en que hay que generar
dinámicamente el registro y eso depende de nuestro provedor de |DNS|\ [#]_.

La documentación de acme.sh_ tiene un `documento donde repasa cómo usar con
acmes.sh las API de distintos provedores de DNS
...
:ref:`desafío DNS <desafios-acme>` se limita a::
   ...
   # acme.sh --issue -d example.com -d '*.example.com' --dns dns_ovh
...
-----------------------------
:program:`certbot` es otro cliente |ACME| compatible con ambas autoridades de
certificación, pero escrita en *Python* y con algunas dependencias que muy
probablemente sólo requiramos en el servidor para poder ejecutarlo. Esta razón
es la que lo hace en principio menos atractivo que el propuesto inicialmente de
ahí que releguemos su explicación a este apartado adicional.
...
.. [#] En puridad no necesitamos aún el servidor puesto que el *script*
   con la opción ``--standalone`` es capaz de ponerse a escuchar en el puerto **80**
   y atender la petición del servidor |ACME|. Pero como de todos modos hay que acabar
   levantando el servidor *web*, se prefiere montarlo antes de obtener el
