...
.. warning:: El método de instalación que se propone consiste en generar un
   ...
   es necesario hacer::
      ...
      # apt install devscripts unzip

   que, como verá, instala bastantes paquetes.

Aunque el propio *script* proporciona un método de instalación, es más
conveniente por pulcritud que procuremos crear un paquete de *Debian*, Para
ello, existe `otro repositorio <https://github.com/octopuce/acme.sh-debian>`_
que nos permite generar el paquete que deseamos. El problema, a su vez, de este
repositorio es que a fecha de redacción de este texto, utiliza :ref:`cron
<cron>` en vez de los :ref:`Timers de SystemD <systemd-timer>`, por lo que en
vez de directamente éste, utilizaremos `el fork del autor de este manual
<https://github.com/sio2sio2/acme.sh-debian>`_. Basta con descargar el
repositorio (por ejemplo el :kbd:`zip`) y::

   # unzip acme.sh-debian-master.zip
   # cd acme.sh-debian-master

Antes de proceder a la generación del paquete es conveniente comprobar en :file:`debian/rules`
el valor de la variable ``BRANCH``:

.. code-block:: bash

   BRANCH = 3.0.6

y modificarla al valor de la última versión de :command:`acme.sh` que veamos en
el repositorio oficial. Una vez hecho eso, basta con::
...
Como hemos generado el paquete la instalación es sumamente sencilla::
   ...
   # cd ..
   # apt install ./acme-sh_3.0.6-2~sio2sio2_all.deb

El paquete instala el *software*, de manera que los certificados generados se
almacenarán dentro del directorio :file:`/etc/acme.sh/` gracias a un alias
incluido en :file:`/root/.bashrc` y crea la tarea periódica necesaria para que
la renovación de los certificados se haga automáticamente.

Si su intención es generar inmediatamente el certificado sin salir de la sesión
de administrador, entonces deberá cargar el *alias* con::
