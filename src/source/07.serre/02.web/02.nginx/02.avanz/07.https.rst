...
#. Mediante el protocolo |ACME|, inicialmente propuesto como estándar por `Let's
   ...
   :dfn:`Desafío HTTP`
      ...
      que se pretende acreditar (p.e. ``www.example.net``) y, si lo obtiene,
...
Una vez generado el certificado, debemos instalarlo::
   ...
   # acme.sh --install-cert -d example.net --key-file /etc/ssl/private/letsencrypt-example.net.key \
      --fullchain-file /etc/ssl/certs/letsencrypt-fullchain-example.net.cer
...
depende de cuál sea nuestro provedor de |DNS|\ [#]_.
...
.. warning:: Este epígrafe se incluye, exclusivamente, por haber sido escrito su
   contenido con anterioridad a gran parte de la exposicíón sobre certificados
