...
tres sabores\ [#]_. Además, cada sabor podía tener como dependencia paquetes de
módulos cargados dinámicamente para forzar su instalación y su carga.
...
del servidor con un número mínimo de módulos compilados estáticamente, y los
cuatro sabores ya mencionados dependen de él y añaden dependencias a paquetes de
módulos que se cargan dinámicamente. En consecuencia, la diferencia entre los
distintos sabores son las dependencias que, en cualquier caso, pueden instalarse
a mano.

.. note:: En estos apuntes optaremos por instalar el paquete base :deb:`nginx`,
   ya que para desarrollar casi todo su contenido basta con él.

.. warning:: Si trabaja con una versión anterior a *Bookworn* es posible que le
   salga más a cuenta instalar :deb:`nginx-light`.
...
.. [#] Estos son los inconvenientes de que los módulos debieran compilarse
   estáticamente.
