...
* Cuando se dispone un servidor |PXE| independiente, éste es el que proporciona
  ...
  *file*, lo cual no tiene por qué haberse producido. De hecho, algunos
  ...
  las variables :kbd:`${pxebs/next-server}` y :kbd:`${pxebs/filename}`. Por ese
  motivo, la expresión anterior del fichero, habría sido mucho más apropiada
  haberla hecho así::

      http://${pxebs/next-server}/boot/start.ipxe

* Abrir una *shell* de |iPXE| puede ayudarnos a conocer mejor cómo ha sido el
  arranque:
  ...
  - La orden :command:`echo` nos da simplemente el valor de una variable, pero
    :command:`show` puede ayudar a saber de dónde toma tal valor::
      ...
      http://${pxebs/next-server}/boot/start.ipxe
      iPXE> show filename
      pxebs/filename:string = http://${pxebs/next-server}/boot/start.ipxe
      iPXE> show next-server
      net0.dhcp/next-server:ipv4 = 192.168.255.1
      iPXE> echo ${pxebs/next-server}
      192.168.255.2
...
* Aunque con la orden :command:`config` no aparecen, las opciones de |DHCP|
  ...
  *script* cargado fuera este\ [#]_::
      ...
      http://${pxebs/next-server}/boot/start.php?mac=${mac}&domain=${15}
...
Es, sin duda, el |NBP| más utilizado y permite una amplica gama de
configuraciones.
